...
////////////////////////////////////////////////////////////////////////////////////
    ...
    var RuntimeClass = function(){
        ...
        self.attributes_to_autohitch = ['width', 'height', 'top', 'left', 'opacity', "right"];
    ...
    // The tagos debugger component will overwrite this to divert messages to it's view
    ...
    Hypertag.Debugger.comment = function(msg){
        var trace = printStackTrace().slice(4).join("\n");
        var msg = String(arguments[0])+"\n";
    ...
    */
    ...
    CompilerClass.prototype.guid = function(){
        return "X"+TrustOS.uuid(16).replace(/\//g, "_").replace(/\+/g, "-");
    };
    ...
    CompilerClass.prototype.ImportIntoTemplateCache = function(template_name){
        ...
        //INTENT: try to get the template node from one of the imported templates
        var templatenode = Hypertag.Runtime.TemplateNodes[template_name] || $('#'+template_name)[0];

        //Finally, if neither worked, the template was not found.
        if(!templatenode)
            throw template_name+" was invoked template of that name has been defined. Check for errors.";
        ...
        self._ConvertHypertagsToInstances(template_dom);
        ...
        //INTENT: determine if the template has no inner xml content. If so, we'll want to mark it
        //so that when we apply it as a trait, we dont incorrectly 
        //if(!template_dom.firstChild)
        //    console.log("empty after recurse", template_dom);
        ...
        //the jquery templates var replacement! what a crock. This sets it back to right -- via string replacement, efficiently -- and ONLY on those attrs
        ...
        while(elem = all_elems.shift()){
            ...
            //if a hypertag, take contents and make a template using import!
                ...
                if(inner_template_nodes.length){
                    ...
                    Hypertag.Runtime.TemplateNodes[uuid] = inner_template;
    ...
    CompilerClass.prototype.ImportTemplatesFromText = function(templatetagstext, overwrite){
        ...
        for(var i in dom.childNodes){
            ...
            if(node.nodeType == 1 && 
               node.tagName == "script" && 
               (node.attributes.getNamedItem("type").nodeValue == "text/hypertag")){
    ...
    CompilerClass.prototype._ConvertHypertagsToInstances = function(template_dom){
        ...
        for(var i = 0, item; (item = itemsready[i]) ; i ++){
            ...
            if(!Hypertag.Runtime.html_tag_names[tagname]){                     
                ...
                var template_name = Hypertag.Runtime.TemplateLowercaseLookup[tagname];
                if(Hypertag.Runtime.CompiledTemplateAttributeNames[template_name])
                    for(var key in Hypertag.Runtime.CompiledTemplateAttributeNames[template_name])
                        new_div.setAttribute(key, Hypertag.Runtime.CompiledTemplateAttributeNames[template_name][key]);
                ...
                if(tagname != 'view'){
                    ...
                    $(new_div).attr("template", template_name);
                }
    ...
    var HypertagClass = function(element, text_of_tag){
        ...
        //this only has meaning in TagOS apps but is best expressed in the runtime. The only exception, in fact.
    ...
    HypertagClass.prototype._performAutohitches = function(){
        var self = this;
        var $self = $(self);
        ...
        while(self._autohitches.length){
            ...
            try{
                ...
                Hypertag.Debugger.error("HypertagOS Engine --------------------------\nHitch Error ------------\n    "+String(e)+"\n\nTarget was: "+item[3]+"\n\nXML of node is"+PrintXML(self));
            }   
        }
    };
    ...
    HypertagClass.prototype._autosizingSetup = function(){
        ...
        var autoduration = parseInt(self.autoduration) || Hypertag.GUI.duration;
        ...
        if(attributes_to_animate.indexOf("top") !== -1){
            listen(self, 'top', function(val){
                ...
                animate(self, {top:val}, autoduration || Hypertag.GUI.duration);
            });
        }
        ...
        if(attributes_to_animate.indexOf("left") !== -1)
            listen(self, 'left', function(val){
                ...
                animate(self, {left:val}, autoduration || Hypertag.GUI.duration);
            });
        ...
        if(attributes_to_animate.indexOf("right") !== -1)
            listen(self, 'right', function(val){
                ...
                animate(self, {right:val}, autoduration || Hypertag.GUI.duration);
            });
        ...
        if(attributes_to_animate.indexOf("opacity") !== -1)
            listen(self, 'opacity', function(val){
                if(String(val).slice(0, 1) == '%') return;
                animate(self, {opacity:parseFloat(val)}, autoduration || Hypertag.GUI.duration);
            });
        ...
        /* or if we are not animating opacity, per normal. */
            ...
            listen(self, 'opacity', function(val){
                if(String(val).slice(0, 1) == '%') return;
                $self.css("opacity", parseFloat(val));
            });
        ...
        if(attributes_to_animate.indexOf("height") !== -1)
            listen(self, 'height', function(val){
                ...
                if("px%".indexOf(String(val).slice(-2)) === -1)
                    val += 'px';
                animate(self, {height:val}, autoduration || Hypertag.GUI.duration);
            });
        ...
        if(attributes_to_animate.indexOf("width") !== -1)
            listen(self, 'width', function(val){
                ...
                if("px%".indexOf(String(val).slice(-2)) === -1)
                    val += 'px';
                animate(self, {width:val}, self.autoduration || Hypertag.GUI.duration);
            });
    ...
    */
            ...
            $.ajax({
                ...
                complete:function(response){
                    try{
                        ...
                        HyXpertag.Debugger.error("Parsing JSON from url '"+self.list+"' had a problem: "+String(err)+". The text was:\n\n"+response.responseText+"\n---- end bad json");
                        return self;
                    }
    ...
    //INTENT: I merely run a selector to find a node beneath self with the given name, a very handy and oft-used shortcut.
    HypertagClass.prototype.jchild = function(names){
        return $(this).jchild(names);
    };
    
    //INTENT: I merely run a selector to find a node beneath self with the given name, a very handy and oft-used shortcut.
    HypertagClass.prototype.jnamed = function(names){
        return $(this).jnamed(names);
    };
    
    /* use this to make some view come front, making all else go back one.  */
    HypertagClass.prototype.jsibling = function(names){
                ...
                //if it's got the magic %%{..} at beginning and end of string...
                    ...
                    if(!hitch_delay_function && hitch_targets.length){
                        ...
                        for(var i = 0; i < hitch_targets.length ; i ++){    
                            ...
                            if(obj_reference && obj_reference.autoanimated && (obj_reference.autoanimated.indexOf(attr) || obj_reference.autoanimated === true || obj_reference.autoanimated.toLowerCase() == 'true' ))
                                maxtime = Math.max(maxtime, obj_reference.autospeed || Hypertag.GUI.duration/2);
                        }
                    ...
                    for(var i = 0; i < hitch_targets.length ; i ++){
                        ...
                        if(hitch_targets[i]){
                            ...
                            if(autohitch_flag || Hypertag.Runtime.attributes_to_autohitch.indexOf(key) !== -1)
...
////////////////////////////////////////////////////////////////////////////////
    ...
    //return a node interior to ourselves named 'name'. This is quicker then looking 
    //for id's as well as being factorable.
    if($.fn['jchild']) throw new Error("$.fn['jchild'] already defined! (hypertag.js trying to redefine it)");
    ...
    //return a node interior to ourselves named 'name'. This is quicker then looking 
    //for id's as well as being factorable.
    if($.fn['jnamed']) throw new Error("$.fn['jnamed'] already defined! (hypertag.js trying to redefine it)");
    ...
    //return a node interior to ourselves named 'name'. This is quicker then looking 
    //for id's as well as being factorable.
    if($.fn['jsibling']) throw new Error("$.fn['jsibling'] already defined! (hypertag.js trying to redefine it)");
    ...
    */
    var post = function(options){
        ...
        if(GLOBAL.fat && membership){
            ...
            if(membership_data)
                url = "http://"+membership_data.servers[0]+url;
        } 
        try{
